...
public class TestHSortInterest4DistanceUltimateEdit implements TAPSolver {
    @Override
        ...
        while (obj.distance(result) <= ist.getMaxDistance() && obj.time(result) <= ist.getTimeBudget()) {
            ...
            if (list.size() > 1) {
                for (int j = 0; j < list.size(); j++) {
                    if (ist.getInterest()[list.get(j)]/ist.getCosts()[list.get(j)] >= ist.getInterest()[index_min]/ist.getCosts()[index_min]) {
